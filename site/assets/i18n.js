...
(function () {
  ...
  const dict = {
    en: {
      ...
      "faq.a6": "Yes. Each provider ships with a sensible default, and you can switch model right from the composer — or in Settings → Model — e.g. Claude Sonnet 4.6, Opus 4.8, or Haiku 4.5. There's also a Custom field to type any model id the provider supports, so you're never stuck waiting for us to add a new one. Custom ids are saved per provider, so they reappear under Saved custom for one-click reuse.",
